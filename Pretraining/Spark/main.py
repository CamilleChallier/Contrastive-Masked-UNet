...
def main_pt():
    ...
    X_pretrain, _, _, _ = train_test_split(X_train, y_train, test_size=args.ratio/0.8, random_state=42)
    ...
    dataset_train = build_dataset_to_pretrain(X_pretrain, args.input_size)
    ...
    if ep_start >= args.ep: # load from a complete checkpoint file
        ...
        for ep in range(ep_start, args.ep):
            ...
            results["train_loss"].append(last_loss)
            min_loss = min(min_loss, last_loss)
            performance_desc = f'{min_loss:.4f} {last_loss:.4f}'
            misc.save_checkpoint_with_meta_info_and_opt_state(f'{args.model}_{args.full_unet}_{args.arcade}_arcade_full_spark_style.pth', args, ep, performance_desc, model_without_ddp.state_dict(with_config=True), optimizer.state_dict())
            misc.save_checkpoint_model_weights_only(f'{args.model}_{args.full_unet}_{args.arcade}_arcade_full_timm_style.pth', args, model_without_ddp.sparse_encoder.sp_cnn.state_dict())
    ...
    import pickle
    with open(f'{args.exp_dir}/result_{args.model}_{args.full_unet}_{args.arcade}_arcade_full.pkl', 'wb') as f:
